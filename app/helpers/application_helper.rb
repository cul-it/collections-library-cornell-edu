...
#    @toc = JSON.parse(HTTPClient.get_content("http://rossini.cul.columbia.edu/voyager_backend/holdings/retrieve/#{params[:id]}"))[params[:id]]
     ...
     toc = clnt.get_content("http://hydrastg.library.cornell.edu/solr/nuremberg_core/select?q=id%3Anur*"+seapage+"_*+AND+head_tesim%3A*&rows=1000&sort=id+asc&fl=head_tesim%2C+id%2C+node_tesim%2C+image_seq_tesim&wt=ruby") # do |chunk|
       tocArray = eval(toc)
       i = 0
